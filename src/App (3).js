...
function LandingPage({ onGetStarted, onLegal }) {
  const tools = [
    ...
    { emoji:"🏋️", label:"S&C Programme",    tagline:"Strength & conditioning with exercise images 🔒 Pro", color:"#e879f9" },
  ];
...
// ── S&C Programme ──────────────────────────────────────────────────────────
const SC_FOCUS = ["💨 Speed & Agility","💪 Strength & Power","🫀 Endurance","🦵 Lower Body","🤸 Mobility & Flexibility","🛡 Injury Prevention","⚡ Explosive Power","🧘 Core Stability"];
...
function SCProgramme({ userId, showToast, goHome }) {
  ...
  const [loading,  setLoading]  = useState(false);
  const [exercises, setExercises] = useState(null); // parsed list
  const [rawText,  setRawText]  = useState("");
  const [error,    setError]    = useState("");
  const [imgCache, setImgCache] = useState({}); // exerciseName -> imgUrl

  const canGen = sport && ageGroup && level;

  // Parse AI output into exercise objects
  const parseExercises = (text) => {
    const blocks = text.split(/##\s*\d+\.\s+/).filter(Boolean);
    return blocks.map(block => {
      const lines = block.trim().split("\n").filter(Boolean);
      const name = lines[0]?.trim() || "Exercise";
      const rest = lines.slice(1).join("\n");
      return { name, detail: rest };
    });
  };

  const fetchImage = async (exerciseName) => {
    if (imgCache[exerciseName]) return;
    try {
      ...
      // Unsplash demo won't work, so we'll use a reliable exercise image service
      // Use a deterministic placeholder based on exercise name
    } catch {}
  };

  // Get a reliable exercise image URL using exercise name
  const getExerciseImgUrl = (name) => {
    // Use Unsplash Source API with exercise-specific search terms
    const slug = encodeURIComponent(name.toLowerCase().replace(/[^a-z0-9\s]/g,"").trim().split(" ").slice(0,3).join("-"));
    // Fallback to a reliable fitness image source
    const seed = name.split("").reduce((a,c)=>a+c.charCodeAt(0),0);
    const fitnessTerms = ["squat","deadlift","pushup","plank","sprint","lunge","jump","row","press","pull"];
    const term = fitnessTerms[seed % fitnessTerms.length];
    return `https://source.unsplash.com/400x250/?${encodeURIComponent(name + " exercise fitness")}`;
  };

  const generate = async () => {
    if (!canGen) return;
    setLoading(true); setError(""); setExercises(null); setRawText("");
    const prompt = `Create a detailed strength and conditioning programme for ${ageGroup} ${sport.replace(/^[^\w]+/,"").trim()} players at ${level.replace(/^[^\w]+/,"").trim()} level. Focus: ${scFocus||"general athleticism"}. Session duration: ${duration||"60 mins"}.
...
Format each exercise with a header like: ## 1. [Exercise Name]
Then sets/reps, then coaching points, then equipment.
End with a brief warm-up and cool-down section.
Make it practical and age-appropriate for ${ageGroup}.`;
  ...
  const renderDetail = (detail) => {
    return detail.split("\n").map((line,i)=>{
      ...
      return (
        <div key={i} style={{minHeight:line===""?"8px":"auto",fontSize:13,color:"#94a3b8",lineHeight:1.7}}>
          {parts.map((pt,j)=>j%2===1
            ? <span key={j} style={{color:"#f1f5f9",fontWeight:700}}>{pt}</span>
            : <span key={j}>{pt}</span>
          )}
        </div>
      );
    ...
    <div>
      ...
      <div style={{marginBottom:24,marginTop:16}}>
        ...
        <div style={{fontSize:14,color:"#64748b",marginTop:4,display:"flex",alignItems:"center",gap:8}}>
          ...
          <span style={{fontSize:11,fontWeight:700,color:"#e879f9",background:"rgba(232,121,249,0.12)",border:"1px solid rgba(232,121,249,0.3)",padding:"2px 8px",borderRadius:4,letterSpacing:1}}>🔒 PRO</span>
        </div>
      </div>
      ...
      <div style={{background:"#16202b",border:"1px solid rgba(255,255,255,0.08)",borderRadius:12,padding:24,maxWidth:700,marginBottom:32}}>
        ...
        <button className="gen-btn" onClick={generate} disabled={!canGen||loading}
          style={{background:canGen&&!loading?"#e879f9":"#1c2a38",color:canGen&&!loading?"#1a0020":"#64748b",cursor:canGen&&!loading?"pointer":"not-allowed"}}>
          {loading ? "🏋️ Building Your Programme…" : "⚡ Generate S&C Programme →"}
        </button>
      </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(300px,1fr))",gap:20,maxWidth:1060}}>
          {[1,2,3,4,5,6].map(i=>(
            <div key={i} style={{background:"#16202b",border:"1px solid rgba(255,255,255,0.08)",borderRadius:12,overflow:"hidden"}}>
              <div style={{height:180,background:"linear-gradient(90deg,#1c2a38 25%,#243040 50%,#1c2a38 75%)",backgroundSize:"200% 100%",animation:"shimmer 1.5s infinite"}}/>
      ...
      {exercises && !loading && (
        <div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(300px,1fr))",gap:20,maxWidth:1060,marginBottom:32}}>
            {exercises.map((ex,i)=>(
              <div key={i} style={{background:"#16202b",border:"1px solid rgba(232,121,249,0.15)",borderRadius:12,overflow:"hidden",transition:"transform .2s",cursor:"default"}}
                onMouseEnter={e=>e.currentTarget.style.transform="translateY(-3px)"}
                onMouseLeave={e=>e.currentTarget.style.transform="translateY(0)"}>
                {/* Exercise image */}
                <div style={{position:"relative",height:190,overflow:"hidden",background:"#0d1520"}}>
                  <img
                    src={`https://source.unsplash.com/400x250/?${encodeURIComponent(ex.name.split(" ").slice(0,3).join(" ")+" exercise fitness strength")}`}
                    alt={ex.name}
                  ...
                  <div style={{display:"none",position:"absolute",inset:0,alignItems:"center",justifyContent:"center",background:"linear-gradient(135deg,#1a0020,#2d1040)",fontSize:56}}>
                    🏋️
                  </div>
                  <div style={{position:"absolute",bottom:0,left:0,right:0,height:60,background:"linear-gradient(transparent,rgba(22,32,43,0.95))"}}/>
                  <div style={{position:"absolute",bottom:12,left:14,fontFamily:"'Barlow Condensed',sans-serif",fontSize:11,fontWeight:700,color:"#e879f9",letterSpacing:"1.5px",textTransform:"uppercase"}}>
                    Exercise {i+1}
                  </div>
                </div>
                {/* Content */}
                <div style={{padding:"14px 16px 18px"}}>
                  <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:20,fontWeight:800,textTransform:"uppercase",color:"#f1f5f9",marginBottom:10,lineHeight:1.1}}>
                    {ex.name}
                  </div>
                  <div>{renderDetail(ex.detail)}</div>
                </div>
              </div>
            ))}
          </div>

          {/* Warm-up / cool-down section — extract from rawText */}
          {rawText.includes("Warm") && (
                ...
                {TOOLS.map(t=>{
                  ...
                  return (
                    <button key={t.id} className="tcard" onClick={()=>isLocked?setShowUpgrade(true):openTool(t)} style={{borderColor:"rgba(255,255,255,0.08)",opacity:isLocked?0.75:1}} onMouseEnter={e=>e.currentTarget.style.borderColor=t.color} onMouseLeave={e=>e.currentTarget.style.borderColor="rgba(255,255,255,0.08)"}>
                      ...
                      {isLocked
                        ? <span style={{position:"absolute",top:14,right:14,fontSize:12,fontWeight:700,color:"#e879f9",background:"rgba(232,121,249,0.12)",border:"1px solid rgba(232,121,249,0.3)",padding:"2px 8px",borderRadius:4,letterSpacing:1}}>🔒 PRO</span>
