...
// Renders an interactive pitch where coaches can place cones, balls,
// players, opponents and draw movement/ball-path arrows.
// items: [{type, x, y}]  arrows: [{x1,y1,x2,y2,kind}]
function DrillCanvas({ items, arrows, onChange, readOnly, width }) {
  // tool: "player"|"opponent"|"cone"|"ball"|"move"|"pass"|"delete"|"select"
  const [tool,        setTool]        = useState("player");
  const [drawing,     setDrawing]     = useState(null); // arrow in progress {x1,y1}
  const [selected,    setSelected]    = useState(null); // {type:"item"|"arrow", idx}
  const svgRef = { current: null };

  const W = width || 340;
  const H = Math.round(W * 1.35);

  // Convert mouse/touch event to SVG coords
  const evToSVG = (e) => {
    ...
    return {
      x: Math.round(((touch.clientX - rect.left) / rect.width)  * 100),
      y: Math.round(((touch.clientY - rect.top)  / rect.height) * 100),
    };
  };
  ...
  const handleSVGClick = (e) => {
    if (readOnly) return;
    const {x, y} = evToSVG(e);
    if (tool === "player")   { onChange({ items:[...items, {type:"player",   x, y}], arrows }); setSelected(null); }
    else if (tool === "opponent") { onChange({ items:[...items, {type:"opponent", x, y}], arrows }); setSelected(null); }
    else if (tool === "cone")     { onChange({ items:[...items, {type:"cone",     x, y}], arrows }); setSelected(null); }
    else if (tool === "ball")     { onChange({ items:[...items, {type:"ball",     x, y}], arrows }); setSelected(null); }
  };
  ...
  const handleItemClick = (e, idx) => {
    ...
    if (readOnly) return;
    if (tool === "delete") {
      onChange({ items: items.filter((_,i)=>i!==idx), arrows });
    } else if (tool === "move" || tool === "pass") {
      if (!drawing) {
        setDrawing({ x1: items[idx].x, y1: items[idx].y });
      } else {
        onChange({ items, arrows:[...arrows, { x1:drawing.x1, y1:drawing.y1, x2:items[idx].x, y2:items[idx].y, kind:tool }] });
        setDrawing(null);
      }
    } else if (tool === "select") {
      setSelected({ type:"item", idx });
    }
  };
  ...
  const handleArrowClick = (e, idx) => {
    ...
    if (readOnly) return;
    if (tool === "delete") {
      onChange({ items, arrows: arrows.filter((_,i)=>i!==idx) });
    }
  };

  const handleClear = () => { onChange({ items:[], arrows:[] }); setDrawing(null); setSelected(null); };

  const TOOLS = [
    { id:"player",   label:"🔵", title:"Add player (blue)" },
    { id:"opponent", label:"🔴", title:"Add opponent (red)" },
    { id:"cone",     label:"🟠", title:"Add cone" },
    { id:"ball",     label:"⚽", title:"Add ball" },
    { id:"move",     label:"➡️", title:"Player movement arrow" },
    { id:"pass",     label:"⚡", title:"Ball pass line" },
    { id:"delete",   label:"🗑",  title:"Delete item" },
  ];

  const renderItem = (item, idx) => {
    const size = 14;
    const cx = item.x, cy = item.y;
    const isSelected = selected?.type==="item" && selected.idx===idx;
    let inner = null;
    if (item.type === "player") {
      inner = <circle cx={`${cx}%`} cy={`${cy}%`} r="3.2%" fill="url(#playerGrad)" stroke={isSelected?"#fbbf24":"rgba(255,255,255,0.9)"} strokeWidth="0.8%" style={{cursor:readOnly?"default":"pointer"}} onClick={e=>handleItemClick(e,idx)}/>;
    } else if (item.type === "opponent") {
      inner = <circle cx={`${cx}%`} cy={`${cy}%`} r="3.2%" fill="url(#oppGrad)" stroke={isSelected?"#fbbf24":"rgba(255,255,255,0.9)"} strokeWidth="0.8%" style={{cursor:readOnly?"default":"pointer"}} onClick={e=>handleItemClick(e,idx)}/>;
    } else if (item.type === "cone") {
      // Triangle
      const px = cx/100, py = cy/100;
      const pts = `${cx}%,${cy-4}% ${cx-2.5}%,${cy+2}% ${cx+2.5}%,${cy+2}%`;
      inner = <polygon points={pts} fill="#f97316" stroke="rgba(255,255,255,0.8)" strokeWidth="0.5%" style={{cursor:readOnly?"default":"pointer"}} onClick={e=>handleItemClick(e,idx)}/>;
    } else if (item.type === "ball") {
      inner = (
        <g style={{cursor:readOnly?"default":"pointer"}} onClick={e=>handleItemClick(e,idx)}>
          <circle cx={`${cx}%`} cy={`${cy}%`} r="2.5%" fill="white" stroke="rgba(0,0,0,0.5)" strokeWidth="0.5%"/>
          <line x1={`${cx-2.5}%`} y1={`${cy}%`} x2={`${cx+2.5}%`} y2={`${cy}%`} stroke="rgba(0,0,0,0.3)" strokeWidth="0.4%"/>
          <line x1={`${cx}%`} y1={`${cy-2.5}%`} x2={`${cx}%`} y2={`${cy+2.5}%`} stroke="rgba(0,0,0,0.3)" strokeWidth="0.4%"/>
        </g>
      );
    }
    return <g key={idx}>{inner}</g>;
  };

  const renderArrow = (arr, idx) => {
    const color = arr.kind === "pass" ? "#fbbf24" : "#60a5fa";
    const dash  = arr.kind === "pass" ? "3,2" : "none";
    const dx = arr.x2 - arr.x1, dy = arr.y2 - arr.y1;
    const len = Math.sqrt(dx*dx + dy*dy);
    if (len < 1) return null;
    const ux = dx/len, uy = dy/len;
    // Shorten line to not overlap tokens (r≈3.2% of 100 units)
    const gap = 3.5;
    const x1 = arr.x1 + ux*gap, y1 = arr.y1 + uy*gap;
    const x2 = arr.x2 - ux*gap, y2 = arr.y2 - uy*gap;
    // Arrowhead
    const headLen = 3;
    ...
    return (
      <g key={idx} style={{cursor:readOnly?"default":"pointer"}} onClick={e=>handleArrowClick(e,idx)}>
        <line x1={`${x1}%`} y1={`${y1}%`} x2={`${x2}%`} y2={`${y2}%`}
          stroke={color} strokeWidth="1%" strokeDasharray={dash} strokeLinecap="round"/>
    ...
    <div style={{userSelect:"none"}}>
      ...
      {!readOnly && (
        <div style={{display:"flex", gap:4, flexWrap:"wrap", marginBottom:8, alignItems:"center"}}>
          {TOOLS.map(t => (
            <button key={t.id} title={t.title} onClick={()=>{ setTool(t.id); setDrawing(null); }}
              style={{
                padding:"5px 10px", borderRadius:6, border:"1.5px solid "+(tool===t.id?"#fb923c":"rgba(255,255,255,0.1)"),
                background: tool===t.id?"rgba(251,146,60,0.15)":"rgba(255,255,255,0.04)",
                color:"#f1f5f9", fontSize:14, cursor:"pointer",
                boxShadow: tool===t.id?"0 0 0 1px #fb923c inset":"none",
              }}>
              {t.label}
            </button>
          ))}
          <button onClick={handleClear} title="Clear all"
            style={{padding:"5px 10px", borderRadius:6, border:"1px solid rgba(248,113,113,0.3)", background:"transparent", color:"#f87171", fontSize:13, cursor:"pointer", marginLeft:"auto"}}>
            Clear
          </button>
        </div>
      )}
      {drawing && <div style={{fontSize:11, color:"#fb923c", marginBottom:6}}>⚡ Now click the destination player/item to complete the arrow</div>}
      {/* SVG Pitch */}
      <svg
        ref={el => { svgRef.current = el; }}
        viewBox="0 0 100 135"
        style={{width:"100%", borderRadius:10, cursor: ["player","opponent","cone","ball"].includes(tool) ? "crosshair" : "default", display:"block"}}
        ...
        {/* Grass */}
        <rect x="0" y="0" width="100" height="135" fill="#256832"/>
        {[0,1,2,3,4,5,6,7,8].map(i=>(
          <rect key={i} x="3" y={3+i*14.3} width="94" height="7.15" fill={i%2===0?"rgba(0,0,0,0.08)":"transparent"}/>
        ))}
        {/* Pitch lines */}
        ...
        <rect x="22" y="3" width="56" height="18" fill="none" stroke="rgba(255,255,255,0.5)" strokeWidth="0.6"/>
        <rect x="35" y="3" width="30" height="7" fill="none" stroke="rgba(255,255,255,0.5)" strokeWidth="0.6"/>
        <rect x="41" y="0.5" width="18" height="3" fill="none" stroke="rgba(255,255,255,0.6)" strokeWidth="0.6"/>
        <rect x="22" y="114" width="56" height="18" fill="none" stroke="rgba(255,255,255,0.5)" strokeWidth="0.6"/>
        <rect x="35" y="125" width="30" height="7" fill="none" stroke="rgba(255,255,255,0.5)" strokeWidth="0.6"/>
        <rect x="41" y="131.5" width="18" height="3" fill="none" stroke="rgba(255,255,255,0.6)" strokeWidth="0.6"/>
        <path d="M 38 21 A 12 12 0 0 1 62 21" fill="none" stroke="rgba(255,255,255,0.5)" strokeWidth="0.6"/>
        <path d="M 38 114 A 12 12 0 0 0 62 114" fill="none" stroke="rgba(255,255,255,0.5)" strokeWidth="0.6"/>
        <circle cx="50" cy="14" r="0.9" fill="rgba(255,255,255,0.5)"/>
        <circle cx="50" cy="121" r="0.9" fill="rgba(255,255,255,0.5)"/>
        {/* Arrows */}
        {arrows.map((arr,i) => renderArrow(arr,i))}
        {/* Items */}
        {items.map((item,i) => renderItem(item,i))}
        {/* Legend */}
        {!readOnly && items.length===0 && arrows.length===0 && (
          <text x="50" y="69" textAnchor="middle" fontSize="4" fill="rgba(255,255,255,0.25)" fontFamily="sans-serif">Click pitch to place items</text>
        )}
      </svg>
      {/* Legend */}
      {!readOnly && (
        <div style={{display:"flex", gap:10, flexWrap:"wrap", marginTop:6, fontSize:11, color:"#64748b"}}>
          <span>🔵 Player</span><span>🔴 Opponent</span><span>🟠 Cone</span><span>⚽ Ball</span>
          <span style={{color:"#60a5fa"}}>➡️ Movement</span><span style={{color:"#fbbf24"}}>⚡ Pass</span>
        </div>
      )}
    </div>
  );
...
function DrillBuilder({ userId, showToast }) {
  ...
  const [diagram,   setDiagram]   = useState({ items:[], arrows:[] });
  const [saving,    setSaving]    = useState(false);
  const [loading,   setLoading]   = useState(true);
  const [expanded,  setExpanded]  = useState(null); // expanded drill id
  ...
  const parseDiagram = (d) => {
    try { return d ? JSON.parse(d) : { items:[], arrows:[] }; }
    catch { return { items:[], arrows:[] }; }
  };
  ...
  return (
    <div style={{ paddingTop:32 }}>
      <div style={{ marginBottom:24 }}>
        ...
        <div style={{ fontSize:14, color:"#64748b", marginTop:4 }}>🛠 Create drills with interactive pitch diagrams</div>
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"minmax(0,1fr) minmax(0,1fr)", gap:24, alignItems:"start" }}>
        ...
        <div>
          <div style={{ background:"#16202b", border:"1px solid rgba(255,255,255,0.08)", borderRadius:12, padding:20, marginBottom:16 }}>
            <Field label="🏷 Drill Name" required fieldId="db-name"><Inp id="db-name" name="drill-name" value={form.name} onChange={e=>setF("name",e.target.value)} placeholder="e.g. Rondo 4v2"/></Field>
            <Field label="🏅 Sport" required fieldId="db-sport"><Sel id="db-sport" name="sport" value={form.sport} onChange={e=>setF("sport",e.target.value)} options={SPORTS} placeholder="Select sport"/></Field>
            <Field label="📝 Description" fieldId="db-desc"><Txt id="db-desc" name="description" value={form.description} onChange={e=>setF("description",e.target.value)} placeholder="Setup and instructions…" rows={3}/></Field>
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
              <Field label="⏱ Duration" fieldId="db-dur"><Inp id="db-dur" name="duration" value={form.duration} onChange={e=>setF("duration",e.target.value)} placeholder="e.g. 10 mins"/></Field>
              <Field label="👥 Players" fieldId="db-players"><Inp id="db-players" name="players" value={form.players} onChange={e=>setF("players",e.target.value)} placeholder="e.g. 8-12"/></Field>
            </div>
            <Field label="🎽 Equipment" fieldId="db-equip"><Inp id="db-equip" name="equipment" value={form.equipment} onChange={e=>setF("equipment",e.target.value)} placeholder="e.g. Bibs, cones, 1 ball"/></Field>
            <button onClick={saveDrill} disabled={saving||!form.name||!form.sport}
              style={{ width:"100%", padding:13, borderRadius:8, border:"none", background:form.name&&form.sport?"linear-gradient(135deg,#ea580c,#fb923c)":"#1c2a38", color:form.name&&form.sport?"#fff":"#64748b", fontFamily:"'Barlow Condensed',sans-serif", fontSize:16, fontWeight:800, textTransform:"uppercase", letterSpacing:1, cursor:form.name&&form.sport?"pointer":"not-allowed", marginTop:8 }}>
        ...
        <div>
          <div style={{ background:"#16202b", border:"1px solid rgba(255,255,255,0.08)", borderRadius:12, padding:16 }}>
            <div style={{ fontSize:11, fontWeight:700, color:"#fb923c", letterSpacing:"1px", textTransform:"uppercase", marginBottom:10 }}>📐 Drill Diagram</div>
            <DrillCanvas items={diagram.items} arrows={diagram.arrows} onChange={setDiagram}/>
          </div>
        </div>
      </div>
      ...
      <div style={{ marginTop:32 }}>
        ...
        {loading
          ...
          : drills.length===0
            ? <div style={{ color:"#64748b", fontSize:14, textAlign:"center", padding:32, border:"1px dashed rgba(255,255,255,0.1)", borderRadius:12 }}>🏗 No drills yet — create your first!</div>
            : <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))", gap:16 }}>
                {drills.map(d=>{
                  const dg = parseDiagram(d.diagram);
                  const hasItems = dg.items.length > 0 || dg.arrows.length > 0;
                  const isOpen = expanded===d.id;
                  return (
                    <div key={d.id} style={{ background:"#16202b", border:"1px solid rgba(255,255,255,0.08)", borderRadius:12, overflow:"hidden" }}>
                      <div style={{ padding:16 }}>
                        <div style={{ display:"flex", justifyContent:"space-between", marginBottom:6 }}>
                          <div>
                            <div style={{ fontWeight:700, fontSize:15, color:"#f1f5f9" }}>⚙️ {d.name}</div>
                            <div style={{ fontSize:12, color:"#fb923c", marginTop:2 }}>{d.sport}{d.duration&&` · ⏱ ${d.duration}`}{d.players_needed&&` · 👥 ${d.players_needed}`}</div>
                          </div>
                          <button onClick={()=>deleteDrill(d.id)} style={{ background:"transparent", border:"1px solid rgba(248,113,113,0.3)", color:"#f87171", borderRadius:6, padding:"4px 10px", fontSize:12, cursor:"pointer", flexShrink:0 }}>🗑</button>
                        </div>
                        {d.description && <div style={{ fontSize:13, color:"#94a3b8", lineHeight:1.6, marginBottom:8 }}>{d.description}</div>}
                        {d.equipment && <div style={{ fontSize:12, color:"#64748b" }}>🎽 {d.equipment}</div>}
                        {hasItems && (
                          <button onClick={()=>setExpanded(isOpen?null:d.id)}
                            style={{ marginTop:10, padding:"5px 12px", borderRadius:6, border:"1px solid rgba(251,146,60,0.3)", background:"rgba(251,146,60,0.06)", color:"#fb923c", fontSize:12, fontWeight:700, cursor:"pointer" }}>
                            {isOpen ? "▲ Hide Diagram" : "📐 View Diagram"}
                          </button>
                        )}
                      </div>
                      {isOpen && hasItems && (
                        <div style={{ borderTop:"1px solid rgba(255,255,255,0.06)", padding:12, background:"rgba(0,0,0,0.2)" }}>
                          <DrillCanvas items={dg.items} arrows={dg.arrows} onChange={()=>{}} readOnly/>
                        </div>
                      )}
                    </div>
                  );
...
export default function GafferAI() {
  ...
  // Auto-generate a simple preset drill diagram based on keywords in the drill text
  const autoPresetDiagram = (drillText) => {
    const t = (drillText || "").toLowerCase();
    const items = [];
    const arrows = [];

    if (t.includes("rondo") || t.includes("possession") || t.includes("circle")) {
      // Circle of players with one in middle
      const ring = [[50,20],[70,35],[70,65],[50,80],[30,65],[30,35]];
      ring.forEach(([x,y]) => items.push({type:"player",x,y}));
      items.push({type:"opponent",x:50,y:50});
      items.push({type:"ball",x:50,y:20});
      arrows.push({x1:50,y1:20,x2:70,y2:35,kind:"pass"});
    } else if (t.includes("shooting") || t.includes("finish") || t.includes("goal")) {
      items.push({type:"player",x:50,y:75},{type:"player",x:35,y:60},{type:"player",x:65,y:60});
      items.push({type:"ball",x:50,y:75});
      arrows.push({x1:50,y1:75,x2:50,y2:30,kind:"move"});
      arrows.push({x1:50,y1:42,x2:50,y2:15,kind:"pass"});
    } else if (t.includes("passing") || t.includes("movement")) {
      items.push({type:"player",x:30,y:70},{type:"player",x:50,y:50},{type:"player",x:70,y:30});
      items.push({type:"cone",x:40,y:60},{type:"cone",x:60,y:40});
      items.push({type:"ball",x:30,y:70});
      arrows.push({x1:30,y1:70,x2:50,y2:50,kind:"pass"});
      arrows.push({x1:50,y1:50,x2:70,y2:30,kind:"pass"});
      arrows.push({x1:30,y1:70,x2:40,y2:60,kind:"move"});
    } else if (t.includes("defend") || t.includes("pressing") || t.includes("press")) {
      items.push({type:"player",x:50,y:65},{type:"player",x:35,y:55},{type:"player",x:65,y:55});
      items.push({type:"opponent",x:50,y:30},{type:"opponent",x:30,y:40},{type:"opponent",x:70,y:40});
      items.push({type:"ball",x:50,y:30});
      arrows.push({x1:50,y1:65,x2:50,y2:45,kind:"move"});
    } else if (t.includes("dribble") || t.includes("1v1") || t.includes("1-v-1")) {
      [30,45,60,75].forEach(x=>items.push({type:"cone",x,y:55}));
      items.push({type:"player",x:50,y:80},{type:"ball",x:50,y:80});
      arrows.push({x1:50,y1:80,x2:30,y2:55,kind:"move"},{x1:30,y1:55,x2:45,y2:55,kind:"move"},{x1:45,y1:55,x2:60,y2:55,kind:"move"});
    } else {
      // Generic: 4 players in a square with ball in centre
      [[30,35],[70,35],[70,65],[30,65]].forEach(([x,y])=>items.push({type:"player",x,y}));
      items.push({type:"ball",x:50,y:50});
      arrows.push({x1:30,y1:35,x2:70,y2:35,kind:"pass"});
    }
    return { items, arrows };
  };

  // Parse training session output into sections, add auto-diagrams per drill
  const renderOutput = (text, toolId) => {
    if (!text) return null;
    if (toolId !== "training") {
      return text.split("\n").map((line,i)=>{
    ...
    // Training session: parse blocks and inject diagrams for each drill
    ...
    lines.forEach((line, i) => {
      ...
      const isDrillHeader =
        /^(drill|exercise|activity|practice|phase|part)\s*\d/i.test(line) ||
        /^\*\*(drill|exercise|activity|warm.?up|cool.?down|main drill|game scenario)/i.test(line) ||
        (/^\*\*/.test(line) && drillCount < 6 && (lower.includes("drill") || lower.includes("exercise") || lower.includes("activity") || lower.includes("warm") || lower.includes("cool") || lower.includes("scenario") || lower.includes("game")));

      if (isDrillHeader) {
        flushBlock();
        drillCount++;
        const diag = autoPresetDiagram(line + " " + (lines.slice(i+1,i+6).join(" ")));
        const headerParts = line.split(/\*\*(.*?)\*\*/g);
        result.push(
          <div key={result.length} style={{margin:"20px 0 8px", padding:"14px 16px 0", background:"rgba(251,146,60,0.05)", border:"1px solid rgba(251,146,60,0.15)", borderRadius:10}}>
            <div style={{fontWeight:700, color:"#fb923c", fontSize:14, marginBottom:10}}>
              {headerParts.map((pt,j)=>j%2===1?<span key={j}>{pt}</span>:<span key={j}>{pt}</span>)}
            </div>
            <div style={{display:"grid", gridTemplateColumns:"1fr 140px", gap:12, alignItems:"start"}}>
              <div style={{fontSize:13, color:"#94a3b8", lineHeight:1.7}}>
                {lines.slice(i+1, i+8).filter(l=>l.trim()).slice(0,5).map((l,j)=>{
                  const p=l.split(/\*\*(.*?)\*\*/g);
                  return <div key={j}>{p.map((pt,k)=>k%2===1?<span key={k} style={{color:"#f1f5f9",fontWeight:700}}>{pt}</span>:<span key={k}>{pt}</span>)}</div>;
                })}
              </div>
              <div style={{flexShrink:0}}>
                <div style={{fontSize:9, fontWeight:700, color:"#fb923c", textTransform:"uppercase", letterSpacing:"0.8px", marginBottom:4}}>📐 Diagram</div>
                <DrillCanvas items={diag.items} arrows={diag.arrows} onChange={()=>{}} readOnly width={130}/>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="output-wrap">
                ...
                <div className="out-panel">
                  ...
                  <div className="out-content">{renderOutput(output, activeTool?.id)}</div>
                </div>
              </div>
            </div>
          )}
