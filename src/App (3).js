...
function AuthScreen({ onAuth }) {
  ...
  const sendOtp = async () => {
    ...
    try {
      ...
      setOtpSent(true);
      setError("");
    } catch (e) { setError(e.message || "Could not send code — check your email address"); }
    setLoading(false);
  };

  const verifyOtp = async () => {
    if (!otpCode || otpCode.length < 8) { setError("Please enter your 8-digit reset code"); return; }
    setLoading(true); setError("");
    try {
      ...
      if (e) throw e;
      // Set flag so main app shows password reset screen
      localStorage.setItem("gafferai_password_reset", "true");
      setOtpVerified(true);
      setError("");
    } catch (e) {
  ...
  const setNewPass = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        setShowReset(false); setOtpSent(false);
        setOtpVerified(false); setOtpCode("");
        setNewPassword(""); setConfirmPassword("");
        setResetSent(false); setError("");
            ...
            <div>
              ...
              {resetSent ? (
                <div style={{ textAlign:"center", padding:"20px 0" }}>
                  ...
                  <div style={{ fontWeight:700, color:"#4ade80", marginBottom:8, fontFamily:"'Barlow Condensed',sans-serif", fontSize:22 }}>Password Updated!</div>
                  <div style={{ fontSize:13, color:"#64748b" }}>Taking you back to login...</div>
                </div>
              ) : otpVerified ? (
                <div>
                  <div style={{ fontSize:13, color:"#4ade80", marginBottom:16, padding:"8px 12px", background:"rgba(74,222,128,0.08)", borderRadius:8 }}>✅ Code verified! Choose your new password.</div>
                  <div style={{ marginBottom:14 }}>
                    <label style={{ display:"block", fontSize:11, fontWeight:700, color:"#94a3b8", letterSpacing:"1.2px", textTransform:"uppercase", marginBottom:6 }}>🔒 New Password</label>
                    <input type="password" value={newPassword} onChange={e=>setNewPassword(e.target.value)} placeholder="At least 6 characters" style={IS} onFocus={fo} onBlur={bl}/>
                  </div>
                  <div style={{ marginBottom:14 }}>
                    <label style={{ display:"block", fontSize:11, fontWeight:700, color:"#94a3b8", letterSpacing:"1.2px", textTransform:"uppercase", marginBottom:6 }}>🔒 Confirm Password</label>
                    <input type="password" value={confirmPassword} onChange={e=>setConfirmPassword(e.target.value)} placeholder="Type it again" onKeyDown={e=>e.key==="Enter"&&setNewPass()} style={IS} onFocus={fo} onBlur={bl}/>
                  </div>
                  ...
                  <button onClick={setNewPass} disabled={loading}
                ...
                <div>
                  <div style={{ textAlign:"center", marginBottom:20 }}>
                    ...
                    <div style={{ fontSize:14, color:"#94a3b8" }}>We sent an 8-digit code to</div>
                    <div style={{ fontSize:14, fontWeight:700, color:"#4ade80", marginTop:4 }}>{email}</div>
                  </div>
                  <div style={{ marginBottom:14 }}>
                    <label style={{ display:"block", fontSize:11, fontWeight:700, color:"#94a3b8", letterSpacing:"1.2px", textTransform:"uppercase", marginBottom:6 }}>🔢 Enter 8-Digit Code</label>
                    <input value={otpCode} onChange={e=>setOtpCode(e.target.value.replace(/[^0-9a-zA-Z]/g,"").slice(0,8))}
                      placeholder="00000000" maxLength={8}
                      onKeyDown={e=>e.key==="Enter"&&verifyOtp()}
                      style={{...IS, fontSize:24, fontWeight:700, letterSpacing:8, textAlign:"center"}}
                      onFocus={fo} onBlur={bl}/>
                  </div>
                  {error && <div style={{ color:"#f87171", fontSize:13, marginBottom:12, padding:"8px 12px", background:"rgba(248,113,113,0.06)", borderRadius:6 }}>⚠️ {error}</div>}
                  <button onClick={verifyOtp} disabled={loading||otpCode.length<8}
                    style={{ width:"100%", padding:13, borderRadius:8, border:"none", background:otpCode.length===8?"linear-gradient(135deg,#16a34a,#4ade80)":"#1c2a38", color:otpCode.length===8?"#052e16":"#64748b", fontFamily:"'Barlow Condensed',sans-serif", fontSize:17, fontWeight:800, textTransform:"uppercase", letterSpacing:1, cursor:otpCode.length===8?"pointer":"not-allowed", marginTop:8 }}>
                ...
                <div>
                  <div style={{ fontSize:13, color:"#64748b", marginBottom:16 }}>Enter your email and we will send you an 8-digit code.</div>
...
export default function GafferAI() {
  ...
  useEffect(()=>{
    ...
    // Check localStorage flag set by OTP verification
    if (localStorage.getItem("gafferai_password_reset") === "true") {
      setShowPasswordReset(true);
    }
  },[]); // eslint-disable-line
  ...
  const needsPasswordReset = showPasswordReset || localStorage.getItem("gafferai_password_reset") === "true";

  if (needsPasswordReset && user) return (
    <PasswordResetScreen
      user={user}
      onDone={() => {
        localStorage.removeItem("gafferai_password_reset");
