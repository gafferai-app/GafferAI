    ...
    try {
      if (mode === "signup") {
        ...
        if (data.user) {
          ...
          // Create the profile row — never block signup if this fails
          try {
          ...
          } catch (_) { /* profile will be created on first login via loadProfile */ }

          // Handle referral bonus (best-effort, never block signup)
          if (refCode) {
            try {
...
export default function GafferAI() {
  ...
  const loadProfile = async (uid) => {
    ...
    if(data) {
      ...
      try { await supabase.from("profiles").upsert(newProfile, { onConflict: "id" }); } catch(_) {}
