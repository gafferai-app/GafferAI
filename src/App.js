  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.88)", backdropFilter:"blur(10px)", zIndex:350, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}>
      <div style={{ width:"100%", maxWidth:420, background:"#111820", border:"2px solid rgba(74,222,128,0.4)", borderRadius:20, padding:32, textAlign:"center" }}>
        ...
        <div style={{ fontFamily:"'Barlow Condensed',sans-serif", fontSize:28, fontWeight:900, textTransform:"uppercase", color:"#f1f5f9", marginBottom:8, lineHeight:1.1 }}>
          You're loving GafferAI!
        </div>
        <div style={{ fontSize:15, color:"#94a3b8", marginBottom:24, lineHeight:1.6 }}>
          You've used <span style={{ color:"#4ade80", fontWeight:700 }}>3 of your 5 free generations</span>. Upgrade to Pro and get <span style={{ color:"#4ade80", fontWeight:700 }}>35 every month</span> — that's a full season of content.
        </div>

        {/* Value props */}
        <div style={{ background:"rgba(74,222,128,0.06)", border:"1px solid rgba(74,222,128,0.15)", borderRadius:12, padding:16, marginBottom:20, textAlign:"left" }}>
          {[
            "✅ 35 AI generations per month",
            "✅ All 8 tools — including S&C Programme",
        ...
        <button onClick={onUpgrade} disabled={!!loading}
          style={{ width:"100%", padding:15, borderRadius:10, border:"none", background: loading ? "#14532d" : "linear-gradient(135deg,#16a34a,#4ade80)", color:"#052e16", fontFamily:"'Barlow Condensed',sans-serif", fontSize:20, fontWeight:900, textTransform:"uppercase", letterSpacing:1, cursor:loading?"not-allowed":"pointer", marginBottom:10 }}>
          {loading ? "⏳ Redirecting to checkout…" : "⭐ Upgrade to Pro — £4.99/mo"}
        </button>
        <div style={{ fontSize:11, color:"#334155", marginBottom:14 }}>🔒 Secure payment via Stripe</div>

        {/* Low-friction starter week option */}
        <div style={{ borderTop:"1px solid rgba(255,255,255,0.08)", paddingTop:14, marginBottom:14 }}>
          <div style={{ fontSize:12, color:"#64748b", marginBottom:10 }}>Not ready to commit? Try it for just £1</div>
          <button onClick={onStarter} disabled={!!loading}
            style={{ width:"100%", padding:12, borderRadius:8, border:"1px solid rgba(245,158,11,0.4)", background:"rgba(245,158,11,0.06)", color:"#f59e0b", fontFamily:"'Barlow Condensed',sans-serif", fontSize:15, fontWeight:700, textTransform:"uppercase", letterSpacing:1, cursor:"pointer", marginBottom:8 }}>
            🎯 Get Starter Week — £1 for 7 days
          </button>
        </div>
...
function ReviewsPage({ user, profile, showToast }) {
  ...
  const isPaid = profile?.plan === "pro" || profile?.plan === "trial" || profile?.plan === "elite";
  ...
  return (
    <div style={{ maxWidth:800, margin:"0 auto", paddingTop:40 }}>
      ...
      <div style={{ display:"flex", background:"rgba(255,255,255,0.04)", borderRadius:10, padding:4, marginBottom:24, gap:4 }}>
        ...
        {tabBtn("submit", isPaid ? (hasReviewed ? "✅ Reviewed" : "✏️ Leave a Review") : "🔒 Leave a Review")}
      </div>
      ...
      {activeTab === "submit" && (
        <div>
          ...
          {user && !isPaid && (
            <div style={{ background:"rgba(251,191,36,0.06)", border:"1px solid rgba(251,191,36,0.2)", borderRadius:12, padding:28, textAlign:"center" }}>
              <div style={{ fontSize:36, marginBottom:12 }}>⭐</div>
              <div style={{ fontFamily:"'Barlow Condensed',sans-serif", fontSize:20, fontWeight:700, color:"#fbbf24", marginBottom:8 }}>Upgrade to Leave a Review</div>
              <div style={{ fontSize:14, color:"#64748b" }}>Only Pro and Elite members can submit reviews.</div>
            </div>
          )}
            ...
            <div>
              ...
              <div className="tool-grid">
                {TOOLS.map(t=>{
                  ...
                  return (
                    <button key={t.id} className="tcard" onClick={()=>isLocked?setShowUpgrade(true):openTool(t)} style={{borderColor:"rgba(255,255,255,0.08)",opacity:isLocked?0.75:1}} onMouseEnter={e=>e.currentTarget.style.borderColor=t.color} onMouseLeave={e=>e.currentTarget.style.borderColor="rgba(255,255,255,0.08)"}>
