...
function TeamSheet({ userId, showToast }) {
  ...
  // playerPositions: array of {name, x, y} — x/y are % of pitch width/height
  const [playerPositions, setPlayerPositions] = useState([]);
  const [dragging,        setDragging]        = useState(null); // index being dragged
  const pitchRef = useState(null);
  const pitchEl  = pitchRef[0];
  const setPitchEl = pitchRef[1];

  const FORMATIONS = ["4-4-2","4-3-3","4-2-3-1","3-5-2","5-3-2","4-5-1","3-4-3","Custom"];

  // Generate default positions on pitch for a formation string e.g. "4-4-2"
  const formationToPositions = (f) => {
    if (!f || f === "Custom") return [];
    const nums = f.split("-").map(Number);
    if (nums.some(isNaN) || nums.length < 2) return [];

    const lines = [1, ...nums]; // GK + each outfield line
    const totalRows = lines.length;
    const positions = [];

    lines.forEach((count, rowIdx) => {
      // y: GK at bottom (85%), attack at top (10%)
      ...
      for (let i = 0; i < count; i++) {
        ...
        positions.push({ name: "", x: Math.round(x), y: Math.round(y), pos: posLabel });
      }
    });
    return positions;
  };
  ...
  useEffect(() => {
    setPlayerPositions(formationToPositions(activeFormation));
  }, [formation, customFormation]); // eslint-disable-line
  ...
  const onPitchDrop = (e) => {
    e.preventDefault();
    if (dragging === null || !pitchEl) return;
  ...
  return (
    <div style={{ paddingTop:32 }}>
      ...
      <div style={{ marginBottom:24 }}>
        ...
        <div style={{ fontSize:14, color:"#64748b", marginTop:4 }}>Drag players to position them on the pitch</div>
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"340px 1fr", gap:24, alignItems:"start" }}>
        ...
        <div>
          <div style={{ background:"#16202b", border:"1px solid rgba(255,255,255,0.08)", borderRadius:12, padding:20, marginBottom:16 }}>
            <Field label="Team Name" required fieldId="ts-name">
              <Inp id="ts-name" value={sheetName} onChange={e=>setSheetName(e.target.value)} placeholder="e.g. U12s vs Town FC"/>
            </Field>
            <Field label="Sport" fieldId="ts-sport">
              <Sel id="ts-sport" value={sport} onChange={e=>setSport(e.target.value)}
            ...
            <Field label="Notes" fieldId="ts-notes">
              <Txt id="ts-notes" value={notes} onChange={e=>setNotes(e.target.value)} placeholder="e.g. Jamie captain, high press..." rows={2}/>
            </Field>
            <button onClick={saveSheet} disabled={saving||!sheetName}
              style={{ width:"100%", padding:13, borderRadius:8, border:"none", background:sheetName?"linear-gradient(135deg,#059669,#34d399)":"#1c2a38", color:sheetName?"#052e16":"#64748b", fontFamily:"'Barlow Condensed',sans-serif", fontSize:15, fontWeight:800, textTransform:"uppercase", letterSpacing:1, cursor:sheetName?"pointer":"not-allowed" }}>
                ...
                {playerPositions.map((p,i)=>(
                  <div key={i} style={{ display:"flex", alignItems:"center", gap:8 }}>
                    ...
                    <input value={p.name||""} onChange={e=>updatePlayerName(i,e.target.value)}
                      placeholder="Player name" style={{...IS,padding:"6px 10px",fontSize:12}} onFocus={fo} onBlur={bl}/>
                  </div>
                ))}
              </div>
            </div>
        ...
        {/* RIGHT: Pitch + saved sheets */}
        <div>
          {/* Pitch */}
          <div
            ref={el=>setPitchEl(el)}
            ...
            <svg viewBox="0 0 100 135" style={{ position:"absolute",inset:0,width:"100%",height:"100%",pointerEvents:"none" }} xmlns="http://www.w3.org/2000/svg">
              {/* Pitch outline */}
              <rect x="3" y="3" width="94" height="129" rx="1" fill="none" stroke="rgba(255,255,255,0.5)" strokeWidth="0.6"/>
              {/* Halfway line */}
              <line x1="3" y1="67.5" x2="97" y2="67.5" stroke="rgba(255,255,255,0.5)" strokeWidth="0.6"/>
              {/* Centre circle */}
              <circle cx="50" cy="67.5" r="10" fill="none" stroke="rgba(255,255,255,0.5)" strokeWidth="0.6"/>
              ...
              {/* Top penalty spot */}
              <circle cx="50" cy="14" r="0.8" fill="rgba(255,255,255,0.5)"/>
              {/* Top penalty arc */}
              ...
              {/* Bottom goal */}
              <rect x="41" y="131" width="18" height="3" fill="none" stroke="rgba(255,255,255,0.6)" strokeWidth="0.6"/>
              {/* Bottom penalty spot */}
              <circle cx="50" cy="121" r="0.8" fill="rgba(255,255,255,0.5)"/>
              {/* Bottom penalty arc */}
              <path d="M 38 114 A 12 12 0 0 0 62 114" fill="none" stroke="rgba(255,255,255,0.5)" strokeWidth="0.6"/>
              {/* Corner arcs */}
              <path d="M 3 6 A 3 3 0 0 1 6 3" fill="none" stroke="rgba(255,255,255,0.4)" strokeWidth="0.5"/>
              <path d="M 94 3 A 3 3 0 0 1 97 6" fill="none" stroke="rgba(255,255,255,0.4)" strokeWidth="0.5"/>
              <path d="M 3 129 A 3 3 0 0 0 6 132" fill="none" stroke="rgba(255,255,255,0.4)" strokeWidth="0.5"/>
              <path d="M 97 129 A 3 3 0 0 1 94 132" fill="none" stroke="rgba(255,255,255,0.4)" strokeWidth="0.5"/>
              {/* Grass stripes */}
              {[0,1,2,3,4,5,6,7,8].map(i=>(
                <rect key={i} x="3" y={3+i*14.3} width="94" height="7.15" fill={i%2===0?"rgba(0,0,0,0.06)":"transparent"}/>
              ))}
              {/* Watermark */}
              <text x="50" y="130.5" textAnchor="middle" fontSize="2.8" fill="rgba(255,255,255,0.35)" fontFamily="sans-serif" fontWeight="600">
            ...
            {playerPositions.map((p,i)=>(
              <div key={i}
                ...
                <div style={{
                  ...
                  background: i===0 ? "linear-gradient(135deg,#fbbf24,#f59e0b)" : "linear-gradient(135deg,#34d399,#059669)",
          ...
          {loading ? <div style={{ color:"#64748b", fontSize:14 }}>Loading...</div>
            : sheets.length===0 ? <div style={{ color:"#64748b", fontSize:13, textAlign:"center", padding:24, border:"1px dashed rgba(255,255,255,0.1)", borderRadius:12 }}>No sheets saved yet</div>
            : <div style={{ display:"flex", flexDirection:"column", gap:8, maxHeight:320, overflowY:"auto" }}>
                {sheets.map(s=>(
                  <div key={s.id} style={{ background:"#16202b", border:"1px solid rgba(255,255,255,0.08)", borderRadius:10, padding:"12px 14px", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                    <div style={{ cursor:"pointer" }} onClick={()=>{
                      try {
                        const loaded = JSON.parse(s.players);
                        setPlayerPositions(loaded);
                        setSheetName(s.name);
                        setSport(s.sport);
