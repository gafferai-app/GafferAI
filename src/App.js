    ...
    try {
      if (mode === "signup") {
        ...
        if (data.user) {
          ...
          // Try to create the profile row — silently ignore errors (RLS/duplicate)
          // loadProfile will create a fallback row if this fails
          try {
          ...
          } catch (_) { /* RLS may block this — loadProfile handles it */ }
          ...
          if (data.session) {
            ...
            // Email confirmation required — tell user and switch to login tab
            setLoading(false);
