...
// Parse a formation string like "4-3-3" into position labels
...
return positions.slice(0, total + 1);
};
...
// Reset players when formation changes
useEffect(() => {
...
if (error) { showToast("Error saving — check Supabase table has formation column"); setSaving(false); return; }
...
<Inp value={sheetName} onChange={e=>setSheetName(e.target.value)} placeholder="e.g. U12s vs Town FC — Sat"/>
</Field>
<Field label="🏅 Sport">
<Sel value={sport} onChange={e=>setSport(e.target.value)} options={["Football","Rugby Union","Rugby League","Netball","Hockey","Cricket","Basketball","Athletics","Tennis","Volleyball"]} placeholder="Sport"/>
</Field>
...
```
      ...
      <Field label="🗂 Formation">
        ...
        {formation==="Custom"&&(
          <input value={customFormation} onChange={e=>setCustomFormation(e.target.value)}
            placeholder="e.g. 4-1-2-1-2" style={{...IS, marginTop:6}}
      ...
      {positions.length > 0 && (
        <div style={{ marginBottom:14, padding:"12px", background:"rgba(52,211,153,0.05)", border:"1px solid rgba(52,211,153,0.1)", borderRadius:8 }}>
      ...
      <Field label="📝 Notes">
        <Txt value={notes} onChange={e=>setNotes(e.target.value)} placeholder="e.g. Jamie captain, press high…" rows={2}/>
      </Field>
  ...
  <div>
    ...
    {loading ? <div style={{ color:"#64748b", fontSize:14 }}>Loading...</div>
      : sheets.length===0
        ? <div style={{ color:"#64748b", fontSize:14, textAlign:"center", padding:32, border:"1px dashed rgba(255,255,255,0.1)", borderRadius:12 }}>No sheets yet!</div>
        : <div style={{ display:"flex", flexDirection:"column", gap:10, maxHeight:600, overflowY:"auto" }}>
            {sheets.map(s=>(
              <div key={s.id}
                style={{ background:"#16202b", border:`1px solid ${viewing?.id===s.id?"#34d399":"rgba(255,255,255,0.08)"}`, borderRadius:12, padding:16, cursor:"pointer" }}
                ...
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                  <div>
                    ...
                    <div style={{ fontSize:12, color:"#34d399", marginTop:2 }}>
                      🏅 {s.sport} {s.formation && `· ${s.formation}`}
                    </div>
                  </div>
                  <button onClick={e=>{e.stopPropagation();deleteSheet(s.id);}}
                    style={{ background:"transparent", border:"1px solid rgba(248,113,113,0.3)", color:"#f87171", borderRadius:6, padding:"4px 10px", fontSize:12, cursor:"pointer" }}>
                    Delete
                  </button>
                </div>
...
export default function GafferAI() {
  ...
  useEffect(()=>{
    supabase.auth.getSession().then(({data:{session}})=>{ if(session?.user){setUser(session.user);loadProfile(session.user.id);} setAuthLoading(false); });
    const {data:{subscription}} = supabase.auth.onAuthStateChange((_,session)=>{ if(session?.user){setUser(session.user);loadProfile(session.user.id);}else{setUser(null);setProfile(null);} });
  ...
  const generate = async () => {
    ...
    try {
      ...
      setOutput(result);
      setHistory(h=>[{id:Date.now(),toolId:activeTool.id,label:activeTool.label,emoji:activeTool.emoji,color:activeTool.color,preview:result.replace(/\*\*/g,"").slice(0,80)+"…",full:result,date:new Date().toLocaleDateString("en-GB")},...h].slice(0,30));
      setScreen("output");
     } catch(e) { setError("Generation failed: " + (e.message || "check your connection and try again.")); }
    setLoading(false);
  };

  const copy   = async () => { await navigator.clipboard.writeText(output); setCopied(true); setTimeout(()=>setCopied(false),2000); };
  const logout = async () => { await supabase.auth.signOut(); setUser(null); setProfile(null); goHome(); };
