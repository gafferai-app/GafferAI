...
function TeamSheet({ userId, showToast }) {
  ...
  const saveSheet = async () => {
    if (!sheetName) return;
    setSaving(true);
    // Save current playerPositions which includes dragged x/y AND names
    const { error } = await supabase.from("team_sheets").insert({
      user_id: userId, name: sheetName, sport,
      formation: activeFormation,
      players: JSON.stringify(playerPositions), // full {name,x,y,pos} objects
    ...
    // Reset form but keep positions so user can keep editing
        ...
        <div key={i}
          ...
          {p.name && (
            <div style={{
              ...
              color:"#fff", fontSize: isMobile ? 9 : 11, fontWeight:800,
              padding:"2px 6px", borderRadius:4,
              whiteSpace:"nowrap", maxWidth: isMobile ? 65 : 84,
