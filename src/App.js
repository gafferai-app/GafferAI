...
function TeamSheet({ userId, showToast }) {
  ...
  const [saving,          setSaving]          = useState(false);
  const [loading,         setLoading]         = useState(true);
  // playerPositions: array of {name, x, y, pos} — x/y are % of pitch
  const [playerPositions, setPlayerPositions] = useState([]);
  const [dragging,        setDragging]        = useState(null);
  const [isMobile,        setIsMobile]        = useState(window.innerWidth < 768);
  // Use a real ref for the pitch element
  ...
  const formationToPositions = (f, existingPositions) => {
    ...
    lines.forEach((count, rowIdx) => {
      ...
      for (let i = 0; i < count; i++) {
        ...
        // Preserve existing name if same slot exists
  ...
  // Only reset positions when formation changes, preserving names where possible
  ...
  const saveSheet = async () => {
    ...
    // Optimistically prepend to list — no need to refetch
  ...
  const onPitchDrop = (e) => {
    ...
    setPlayerPositions(prev => {
      const next = [...prev];
      next[dragging] = { ...next[dragging], x, y };
      return next;
    });
    setDragging(null);
  };
  ...
  const onTouchEnd = (e, idx) => {
    ...
    if (touch.clientX < rect.left || touch.clientX > rect.right ||
      ...
      touchDragging.current = null; return;
    }
    ...
    setPlayerPositions(prev => {
      const next = [...prev];
      next[touchDragging.current] = { ...next[touchDragging.current], x, y };
      return next;
    });
  ...
  const pitch = (
    <div
      ...
      <svg viewBox="0 0 100 135" style={{ position:"absolute",inset:0,width:"100%",height:"100%",pointerEvents:"none" }} xmlns="http://www.w3.org/2000/svg">
        ...
        {/* Watermark */}
        <text x="50" y="130.5" textAnchor="middle" fontSize="2.6" fill="rgba(255,255,255,0.4)" fontFamily="sans-serif" fontWeight="600" letterSpacing="0.3">
          {sheetName || "Team Sheet"} · Built on GafferAI
        </text>
      </svg>
        ...
        <div key={i}
          ...
          {p.name && (
            <div style={{
              ...
              color:"#fff", fontSize: isMobile ? 11 : 13, fontWeight:900,
  ...
  const controls = (
    <div>
      <div style={{ background:"#16202b", border:"1px solid rgba(255,255,255,0.08)", borderRadius:12, padding:20, marginBottom:16 }}>
        <Field label="Team Name" required fieldId="ts-name">
        ...
        <Field label="Notes" fieldId="ts-notes">
        ...
        <button onClick={saveSheet} disabled={saving||!sheetName}
          style={{ width:"100%", padding:13, borderRadius:8, border:"none", background:sheetName?"linear-gradient(135deg,#059669,#34d399)":"#1c2a38", color:sheetName?"#052e16":"#64748b", fontFamily:"'Barlow Condensed',sans-serif", fontSize:15, fontWeight:800, textTransform:"uppercase", letterSpacing:1, cursor:sheetName?"pointer":"not-allowed", marginTop:4 }}>
      ...
      {playerPositions.length > 0 && (
        <div style={{ background:"#16202b", border:"1px solid rgba(255,255,255,0.08)", borderRadius:12, padding:16 }}>
          <div style={{ fontSize:11, fontWeight:700, color:"#34d399", letterSpacing:"1px", textTransform:"uppercase", marginBottom:10 }}>👥 Player Names & Positions</div>
          <div style={{ display:"flex", flexDirection:"column", gap:6, maxHeight:isMobile?220:300, overflowY:"auto" }}>
            {playerPositions.map((p,i)=>(
              <div key={i} style={{ display:"flex", alignItems:"center", gap:6 }}>
                <input
                  value={p.pos||""}
                  onChange={e=>updatePlayerPos(i,e.target.value)}
                  placeholder="Pos"
                  title="Edit position label"
                  style={{...IS, padding:"6px 8px", fontSize:12, width:54, minWidth:54, flexShrink:0, textAlign:"center", fontFamily:"'Barlow Condensed',sans-serif", fontWeight:700, textTransform:"uppercase", color:"#34d399"}}
                  onFocus={fo} onBlur={bl}/>
                <input
                  value={p.name||""}
                  onChange={e=>updatePlayerName(i,e.target.value)}
                  placeholder="Player name"
                  style={{...IS, padding:"6px 10px", fontSize:13, flex:1}}
                  onFocus={fo} onBlur={bl}/>
              </div>
            ))}
          </div>
      ...
      {loading
        ...
        : sheets.length===0
          ...
          : <div style={{ display:"flex", flexDirection:"column", gap:8, maxHeight:280, overflowY:"auto" }}>
              {sheets.map(s=>(
                <div key={s.id} style={{ background:"#16202b", border:"1px solid rgba(255,255,255,0.08)", borderRadius:10, padding:"12px 14px", display:"flex", justifyContent:"space-between", alignItems:"center", gap:8 }}>
                  <div style={{ cursor:"pointer", flex:1, minWidth:0 }} onClick={()=>{
                    try {
                      const loaded = JSON.parse(s.players);
                      // Support both old format (array of strings) and new format (array of objects)
                      const normalized = loaded.map((p,i) =>
                        typeof p === "string"
                          ? { name:p, x:50, y:50, pos:"P"+(i+1) }
                          : p
                      );
                      ...
                      setNotes(s.notes || "");
                      showToast("📋 Sheet loaded onto pitch!");
                    } catch(e) { showToast("Could not load sheet"); }
                  }}>
                    <div style={{ fontWeight:700, fontSize:14, color:"#f1f5f9", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{s.name}</div>
                    <div style={{ fontSize:11, color:"#34d399", marginTop:2 }}>{s.sport} · {s.formation}</div>
                  </div>
                  <button onClick={()=>deleteSheet(s.id)} style={{ background:"transparent", border:"1px solid rgba(248,113,113,0.3)", color:"#f87171", borderRadius:6, padding:"4px 10px", fontSize:11, cursor:"pointer", flexShrink:0 }}>🗑</button>
                </div>
              ))}
            </div>
      }
    ...
    <div style={{ paddingTop:32 }}>
      <div style={{ marginBottom:24 }}>
        ...
        <div style={{ fontSize:14, color:"#64748b", marginTop:4 }}>
          {isMobile ? "👆 Press and hold a player token to drag them on mobile" : "🖱 Drag player tokens to position them on the pitch"}
        </div>
      </div>

      {isMobile ? (
        /* Mobile: pitch full width on top, controls below */
        <div>
          {pitch}
          {controls}
          <div style={{ marginTop:20 }}>{savedSheets}</div>
        </div>
      ) : (
        /* Desktop: controls left, pitch right */
        <div style={{ display:"grid", gridTemplateColumns:"320px 1fr", gap:24, alignItems:"start" }}>
          <div>
            {controls}
            <div style={{ marginTop:0 }}>{savedSheets}</div>
          </div>
          <div>{pitch}</div>
        </div>
      )}
