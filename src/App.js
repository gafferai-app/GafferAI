...
function TeamSheet({ userId, showToast }) {
  ...
  const saveSheet = async () => {
    ...
    if (editingId) {
      ...
      // INSERT new
      const newSheet = { user_id: userId, ...payload, created_at: new Date().toISOString() };
      const { data: inserted, error } = await supabase.from("team_sheets").insert(newSheet).select().single();
      if (error) { showToast("Error saving sheet"); setSaving(false); return; }
      showToast("✅ Team sheet saved!");
      setSheets(prev => [inserted || { ...newSheet, id: Date.now() }, ...prev]);
  ...
  const loadSheet = (s) => {
    try {
      ...
      setNotes(s.notes || "");
      setCoachingPoints(s.coaching_points || "");
      setEditingId(s.id);
              ...
              {sheets.map(s=>(
                <div key={s.id} style={{ background: editingId===s.id ? "rgba(52,211,153,0.06)" : "#16202b", border:`1px solid ${editingId===s.id?"rgba(52,211,153,0.4)":"rgba(255,255,255,0.08)"}`, borderRadius:10, padding:"12px 14px", display:"flex", justifyContent:"space-between", alignItems:"center", gap:8 }}>
                  <div style={{ cursor:"pointer", flex:1, minWidth:0 }} onClick={()=>loadSheet(s)}>
                    ...
                    <div style={{ fontSize:11, color:"#34d399", marginTop:2 }}>{s.sport} · {s.formation}{s.coaching_points?" · 🎯 has coaching points":""}</div>
                  </div>
