  ...
  const submit = async () => {
    ...
    try {
      if (mode === "signup") {
        ...
        // data.user is always returned; data.session is null when email confirmation is required
        if (data.user) {
          const generationsLimit = refCode ? 10 : 5;

          // Use upsert to safely create the profile (avoids duplicate errors if row exists)
          ...
          if (refCode) {
            await supabase.from("referrals").insert({
              ...
              completed: true,
              created_at: new Date().toISOString(),
            }).then(() => {}); // ignore errors if referral already exists
...
function ChangePasswordModal({ onClose, showToast }) {
  ...
  const handleChange = async () => {
    ...
    try {
      ...
      if (e) throw e;
      // Close first, then toast — avoids setState-on-unmounted-component
      onClose();
      setTimeout(() => showToast("✅ Password changed successfully!"), 100);
      return;
    } catch (e) {
    ...
    <div style={{ width:"100%", maxWidth:400, background:"#111820", border:"1px solid rgba(255,255,255,0.12)", borderRadius:20, padding:32 }}>
      ...
      <div style={{ marginBottom:14 }}>
        ...
        <input type="password" value={newPassword} onChange={e=>setNewPassword(e.target.value)}
          placeholder="Min. 6 characters" style={IS} onFocus={fo} onBlur={bl}/>
      </div>
      <div style={{ marginBottom:14 }}>
        <label style={{ display:"block", fontSize:11, fontWeight:700, color:"#94a3b8", letterSpacing:"1.2px", textTransform:"uppercase", marginBottom:6 }}>✅ Confirm New Password</label>
...
export default function GafferAI() {
  ...
  const loadProfile = async (uid) => {
    ...
    if(data) {
      ...
      // Profile missing — create a default one (e.g. email confirmation flow)
      const { data: { user: authUser } } = await supabase.auth.getUser();
      const newProfile = { id: uid, email: authUser?.email || "", plan: "free", generations_used: 0, generations_limit: 5, created_at: new Date().toISOString() };
