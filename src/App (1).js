...
function LegalPage({ page, onClose }) {
  ...
  const terms = (
    <div>
      ...
      <p style={S.p}>The Pro Plan (£4.99/month) renews automatically each month. The Starter Week (£1) is a one-time payment. You may cancel your subscription at any time through your account settings or by contacting us.</p>
  ...
  const submit = async () => {
    ...
    try {
      if (mode === "signup") {
        ...
        if (e) throw e;
        if (data.user) {
          // Give bonus gens if referred
          const bonusGens = refCode ? 10 : 5;
          await supabase.from("profiles").insert({ id:data.user.id, email, generations_used:0, generations_limit:bonusGens, plan:"free", created_at:new Date().toISOString() });
          
          // Process referral if code exists
          if (refCode) {
            // Find referrer by their ref code (first 8 chars of their user id)
            const { data: profiles } = await supabase.from("profiles").select("id, generations_limit").filter("id", "like", refCode + "%");
            if (profiles && profiles.length > 0) {
              const referrer = profiles[0];
              // Give referrer 5 bonus generations
              await supabase.from("profiles").update({ generations_limit: (referrer.generations_limit || 5) + 5 }).eq("id", referrer.id);
              // Record the referral
              await supabase.from("referrals").insert({ referrer_id: referrer.id, referred_email: email, referred_id: data.user.id, completed: true, created_at: new Date().toISOString() });
            }
          }
          onAuth(data.user);
        } else {
...
function AdminDashboard({ onClose }) {
  ...
  const loadData = async () => {
    ...
    const estRevenue   = (proUsers * 4.99) + (trialUsers * 1);
...
export default function GafferAI() {
  ...
  const logout = async () => {
    ...
    localStorage.removeItem("gafferai_session");
    await supabase.auth.signOut();
    setUser(null); setProfile(null); setHistory([]); setSessionToken(null); goHome();
  };
      ...
      <div className="app">
        ...
        {showMobileMenu&&(
          <div style={{position:"fixed",inset:0,zIndex:150,background:"rgba(0,0,0,0.7)",backdropFilter:"blur(4px)"}} onClick={()=>setShowMobileMenu(false)}>
            <div style={{position:"absolute",top:60,right:0,width:220,background:"#111820",borderLeft:"1px solid rgba(255,255,255,0.08)",borderBottom:"1px solid rgba(255,255,255,0.08)",padding:"8px 0"}} onClick={e=>e.stopPropagation()}>
              {screen!=="home"&&<button onClick={()=>{goHome();setShowMobileMenu(false);}} style={{width:"100%",padding:"12px 16px",textAlign:"left",background:"transparent",border:"none",color:"#f1f5f9",fontFamily:"'Barlow',sans-serif",fontSize:14,cursor:"pointer",borderBottom:"1px solid rgba(255,255,255,0.06)"}}>🏠 Home</button>}
              <button onClick={()=>{setShowHistory(true);setShowMobileMenu(false);}} style={{width:"100%",padding:"12px 16px",textAlign:"left",background:"transparent",border:"none",color:"#f1f5f9",fontFamily:"'Barlow',sans-serif",fontSize:14,cursor:"pointer",borderBottom:"1px solid rgba(255,255,255,0.06)"}}>📜 History{history.length>0&&` (${history.length})`}</button>
              <button onClick={()=>{setShowReviews(true);setShowMobileMenu(false);}} style={{width:"100%",padding:"12px 16px",textAlign:"left",background:"transparent",border:"none",color:"#f1f5f9",fontFamily:"'Barlow',sans-serif",fontSize:14,cursor:"pointer",borderBottom:"1px solid rgba(255,255,255,0.06)"}}>⭐ Reviews</button>
              <button onClick={()=>{setShowLegal("contact");setShowMobileMenu(false);}} style={{width:"100%",padding:"12px 16px",textAlign:"left",background:"transparent",border:"none",color:"#f1f5f9",fontFamily:"'Barlow',sans-serif",fontSize:14,cursor:"pointer",borderBottom:"1px solid rgba(255,255,255,0.06)"}}>✉️ Contact</button>
              <button onClick={()=>{setShowReferral(true);setShowMobileMenu(false);}} style={{width:"100%",padding:"12px 16px",textAlign:"left",background:"transparent",border:"none",color:"#f1f5f9",fontFamily:"'Barlow',sans-serif",fontSize:14,cursor:"pointer",borderBottom:"1px solid rgba(255,255,255,0.06)"}}>🔗 Refer{referralCount>0&&` (${referralCount})`}</button>
              <button onClick={()=>{setShowUpgrade(true);setShowMobileMenu(false);}} style={{width:"100%",padding:"12px 16px",textAlign:"left",background:"transparent",border:"none",color:"#fbbf24",fontFamily:"'Barlow',sans-serif",fontSize:14,cursor:"pointer",fontWeight:700,borderBottom:"1px solid rgba(255,255,255,0.06)"}}>⚡ Upgrade</button>
              {isAdmin&&<button onClick={()=>{setShowAdmin(true);setShowMobileMenu(false);}} style={{width:"100%",padding:"12px 16px",textAlign:"left",background:"transparent",border:"none",color:"#f472b6",fontFamily:"'Barlow',sans-serif",fontSize:14,cursor:"pointer",borderBottom:"1px solid rgba(255,255,255,0.06)"}}>🔐 Admin</button>}
              <button onClick={()=>{logout();setShowMobileMenu(false);}} style={{width:"100%",padding:"12px 16px",textAlign:"left",background:"transparent",border:"none",color:"#f87171",fontFamily:"'Barlow',sans-serif",fontSize:14,cursor:"pointer"}}>🚪 Log Out</button>
            </div>
          </div>
        )}
