...
function AuthScreen({ onAuth }) {
  ...
  const verifyOtp = async () => {
    if (!otpCode || otpCode.length < 6) { setError("Please enter your reset code"); return; }
    setLoading(true); setError("");
    try {
      ...
      setOtpVerified(true);
      setError("");
    } catch (e) { setError("Invalid or expired code — request a new one"); console.error("OTP error:", e); }
    setLoading(false);
  };
              ...
              {resetSent ? (
                ...
                <div>
                  ...
                  <button onClick={verifyOtp} disabled={loading||otpCode.length<6}
                    style={{ width:"100%", padding:13, borderRadius:8, border:"none", background:otpCode.length>=6?"linear-gradient(135deg,#16a34a,#4ade80)":"#1c2a38", color:otpCode.length===6?"#052e16":"#64748b", fontFamily:"'Barlow Condensed',sans-serif", fontSize:17, fontWeight:800, textTransform:"uppercase", letterSpacing:1, cursor:otpCode.length===6?"pointer":"not-allowed", marginTop:8 }}>
