    ...
    try {
      if (mode === "signup") {
        ...
        if (data.user) {
          ...
          // If referred, update their bonus gens after a short delay
          if (refCode) {
            setTimeout(async () => {
              // Give referred user bonus gens
              await supabase.from("profiles").update({ generations_limit: 10 }).eq("id", data.user.id);
              // Find and reward referrer
              const { data: allProfiles } = await supabase.from("profiles").select("id, generations_limit");
              const referrer = allProfiles?.find(p => p.id.startsWith(refCode));
              if (referrer) {
