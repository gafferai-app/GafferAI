...
function AuthScreen({ onAuth }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      // Hide raw database/internal errors from users — show a friendly message instead
      const msg = e.message || "";
      const isDbError = msg.toLowerCase().includes("database") || msg.toLowerCase().includes("violates") || msg.toLowerCase().includes("duplicate") || msg.toLowerCase().includes("relation");
      setError(isDbError ? "Something went wrong. Please try again or contact support." : msg || "Something went wrong");
    }
    setLoading(false);
  };
